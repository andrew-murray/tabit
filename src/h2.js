...
{
  // deal with single elements (not wrapped in an array) or multiple
  // elements (which appears as an array)
  const instrumentElements = [].concat(dom.song.instrumentList.instrument);
  ...
  // deal with single elements (not wrapped in an array) or multiple
  // elements (which appears as an array)
  const patternElements = [].concat(dom.song.patternList.pattern);
  // patterns
  // [  { name, size, notes } ]
  ...
  if(dom.song.virtualPatternList && dom.song.virtualPatternList.pattern)
  {
    ...
    // deal with single elements (not wrapped in an array) or multiple
    // elements (which appears as an array)
    const virtualPatternGroups = [].concat(dom.song.virtualPatternList.pattern);
